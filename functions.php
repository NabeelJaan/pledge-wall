...
function tailpress_enqueue_scripts() {
   ...
   // wp_enqueue_script('custom-pw', get_template_directory_uri() . '/resources/js/custom-pw.js', array(), '1.0.0', true);
...
function enqueue_add_pledge_scripts() {
    ...
    // Enqueue custom script for handling form submission
    wp_enqueue_script('custom-add-pledge', get_template_directory_uri() . '/resources/js/custom-add-pledge.js', array('jquery'), null, true);
...
//     if ($result === false) {
//         wp_send_json_error(array('message' => 'Pledge not saved.'));
//     } else {
//         $new_pledge_id = $wpdb->insert_id;
//         $new_pledge = $wpdb->get_row("SELECT * FROM $table_name WHERE id = $new_pledge_id");
//         wp_send_json_success($new_pledge);
//     }
// }

// function custom_pw_get_pledges() {
//     global $wpdb;
//     $table_name = $wpdb->prefix . 'pledge_wall';
...
// add_action('wp_ajax_get_pledges', 'custom_pw_get_pledges');
// add_action('wp_ajax_nopriv_get_pledges', 'custom_pw_get_pledges');
...
// echo '<script>';
// echo 'console.log(' . $json_results . ');';
// echo 'console.log(' . $last_id . ');';
// echo '</script>';
if (!empty($results)) {
