...
<main>
    <!-- Pledge Head -->
    <div class="pledge_wall px-4 text-white pb-3 pt-[200px] lg:pt-[250px] lg:pb-3 xl:px-0 bg-cover bg-no-repeat"
        ...
        <div class="max-w-[1280px] mx-auto">
            <div class="flex flex-col gap-6 md:flex-row md:items-center">
                <div class="relative w-ful md:w-[40%] z-30" id="featuredPledge">
                    <?php if (!empty($results)): ?>
                        ...
                        <?php $image_url = wp_get_attachment_url($latestPledge->pledgeImage); ?>
                            <div class="bg-white min-h-[150px] lg:min-h-[300px] p-3 border-2 border-red-600 rounded-md shadow-md">
                                <div class="flex justify-between items-center mb-4">
                                    <h2 class="text-red-600 font-bold" id="featuredUserName"><?php echo esc_html($latestPledge->userName); ?></h2>
                                </div>
                                <?php if (!empty($image_url)): ?>
                                    <img class="w-full object-cover mb-4" id="featuredPledgeImage" src="<?php echo esc_url($image_url); ?>" alt="Pledge Image"><p id="featurenoimg"></p>
                                <?php else: ?>
                                        <img class="w-full object-cover mb-4" id="featuredPledgeImage" src="" alt="Pledge Image">
                                <p id="featurenoimg">
                ...
                <div class="w-full md:w-[60%] relative z-40 ml-auto mb-[-66px]">
    ...
    <!-- Pledge -->
    <div class="max-w-[1280px] px-4 xl:px-0 mx-auto mt-4 pb-10">
        <div class="relative">

            <div class="grid col-span-1 md:grid-cols-3 lg:grid-cols-5 lg:px-0 gap-3 pledgeWall">
                <?php foreach ($results as $index => $pledge): ?>
                    <?php if ($index > 0): ?>
...
function updateFeaturedPledge() {
        ...
        if (pledgeImage) {
			...
			document.getElementById('featurenoimg').style.display = 'block';
        }
    }
    ...
    // Update the featured pledge every 30 seconds
    setInterval(updateFeaturedPledge, 3000);
